    ...
    <div class="related-posts__container">
      <?php foreach ($related_posts as $related_post) { ?>
        ...
        <div class="related-posts__single">
          <?php $link = get_field('external_link', $related_post->ID) ? get_field('link', $related_post->ID) : get_permalink($related_post->ID); ?>
          <article id="post-<?php the_ID($related_post->ID); ?>" <?php post_class('cf post-card'); ?> role="article">

            <header class="article-header">

              <div class="featured-image-container">
                <a href="<?= $link ?>" class="post-link">
                  <img src="<?= get_the_post_thumbnail_url($related_post->ID, 'large') ?>" alt="<?= the_title($related_post->ID) ?>" class="featured-image">
                  <span class="overlay"><i class="far fa-file-alt"></i></span>
                </a>
              </div>

              <h2 class="h2 entry-title">
                <a href="<?= $link ?>" rel="bookmark" title="<?php the_title_attribute($related_post->ID); ?>">
              ...
              <p class="byline entry-meta">
                <?php printf(
                  ...
                  '<time class="updated entry-time" datetime="' . get_the_time('Y-m-d', $related_post->ID) . '" itemprop="datePublished">' . get_the_time(get_option('date_format'), $related_post->ID) . '</time>',
                  /* the categories of the post */
                  '<span class="by">' . __('|', 'bonestheme') . '</span> <span class="post-categories" itemprop="categories">' .  get_the_category_list(', ', '', $related_post->ID) . '</span>'
                ); ?>
              </p>
